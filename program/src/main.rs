...
impl Snake {
    pub fn render(&mut self, args: &RenderArgs) {
        ...
        let squares: Vec<types::Rectangle> = self.snake_parts
            .iter()
            .map(|p| Snake_Piece(p.0 * self.width, p.1 * self.width))
            .map(|p| rectangle::square(p.0 as f64, p.1 as f64, self.width as f64))
            .collect();
...
impl Food {
    ...
    fn render(&mut self, gl: &mut GlGraphics, args: &RenderArgs, width: u32) {
        ...
        let square = rectangle::square(x as f64, y as f64, width as f64);
